...
/**
 * The main evolutionary class. This, is implemented all the evolution logic (The fitness function) to scoring the individuals of the actual population.
 *
...
public class SchedulingStudyPlanProblem extends Problem implements SimpleProblemForm {
	...
	/**
	 * Responsible for to calculate the fitness value of the individual in related of the contraints below. <br/>
	 *
	 * <br/>
	 * Constraints classification: <br/>
	 * <br/>
	 * FIXED: <br/>
	 ...
	 *
	 * @param individual
	 *            the curent individual to be calculated.
	 *
	...
	public float calculateFitnessValue(DayPlanGeneVectorIndividual individual) {
		...
		float soft = (maxHour + differentPlans) / 2;

		float fitness = fixed + (hard * 0.55f) + (soft * 0.15f);
	 ...
	 *
	 * <br/>
	 * Classification: Fixed <br/>
	 *
	 * @param individual
	 *            the current individual to be verified.
	 *
	...
	/**
	 * Get the correct acumulativeValue in relation of the quantity of subjects was alocated in the plan.
	 *
	 * @param qttSubjectsFound
	 *            [description]
	 *
	 * @return <code>float</code> the acumulative value.
	 */
	...
	/**
	 * Subjects that contains more dificulty needs more time to study. <br/>
	 *
	 * <br/>
	 * Classification: Hard Constraint. <br/>
	 *
	 * @param individual
	 *            the current individual to be verified.
	 *
	...
	/**
	 * Get the percent of the real hours of the subject in relation of the ideal hours that this subject should be.
	 *
	 * @param hoursIdeal
	 *            the quantity of ideal hours to be used by the subject analysed.
	 * @param hoursReal
	 *            the quantity of real hours used by the subject analysed.
	 *
	...
	/**
	 * Get the sum of the all workload of the subject in the all study plan.
	 *
	 * @param individual
	 *            the study plan.
	 * @param subject
	 ...
	 *
	 * @param subjectWorkloads
	 *            the set of subjectWorkloads.
	 * @param subjectId
	 *            the id of the subject to be found.
	 *
	 ...
	 */
	/**
	 * Get the total of ideal hours to study a subject with its dificulty percent.
	 *
	 * @param qttHoursAvailable
	 *            the total of hours available to study.
	 * @param percent
	 *            the percent of the subject dificulty.
	 *
	 * @return <code>float</code> the percent of proximity between the params.
	 */
	 ...
	 *
	 * @param individual
	 *            the study plan to be get the qtt of hours available.
	 *
	...
	/**
	 * Get dificulty percent in relation of the sum of all others dificulties.
	 *
	 * @param dificulty
	 *            dificulty to get the sum.
	 * @param dificultySum
	 *            sum of all dificulty.
	 *
	...
	public int getDificultyPercent(int dificulty, int dificultySum) {
		...
		int percent = 0;
		if (dificultySum != 0) {
			percent = (dificulty * 100) / dificultySum;
		}
	...
	/**
	 * Check if the study plan have grow-up learn. To do this, verify the quantity of the medium difficulty subjects exist in the begin of the plan. After
	 * verify the hard and finally the easy difficulty subjects.Then placing one percent that depends the quantity it was found. <br/>
	 *
	 * <br/>
	 * Classification: Hard Constraint. <br/>
	 *
	...
	/**
	 * Count a number of SubjectWorkloads (SW) that exist in the Array of the periods (Array of SW) by init and final indices.
	 *
	 * @param allPeriods
	 *            the Array of periods to be counted.
	 * @param init
	 *            the first position.
	 * @param end
	 *            the last position.
	...
	/**
	 * Count the number of 1 exist in the especific period of the dificulty. <br/>
	 *
	 * <br/>
	 * Obs.: 1 signific that exist one period of the day empty. In the other words, this period not contains SubjectWorkloads.
	 *
	 * @param period
	 *            {MEDIUM, HARD or EASY}
	 * @param periodsEmpty
	 *            the vector of empty periods.
	 *
	...
	/**
	 * Search if the amountFound is the same of the total or is the majority.
	 *
	 * @param amountFound
	 *            the amount to be compair.
	 * @param total
	 *            the total to be compair.
	 *
	 * @return <code>float</code> the percent of proximity between the params.
	 */
	...
	/**
	 * Responsible to count the number of difficulty type that the array of the subjects contains from a especific period of index (from init position to end
	 * position).
	 *
	 * @param allPeriods
	 *            the periods to be counted.
	 * @param init
	 *            the position of the first subjectWorkload.
	 * @param end
	 *            the position of the last subjectWorkload.
	...
	/**
	 * verify if the first number is between the second (included) and third (exclude) number.
	 *
	 *
	 * @param verify
	 ...
	 *            to (excluded).
	 *
	 * @return <code>true</code> if is between. <code>false</code> otherwise.
	 */
	...
	/**
	 * Hard Subjects should be alocated in the period of the day that the user have more intelectual facility for to learn.
	 *
	 * <br/>
	 * Classification: Hard Constraint. <br/>
	 *
	...
	/**
	 * Return the acumulativeValue from the period using the table classification below.
	 *
	 * The subject classification by difficulty: 00 >= n < 20 Easy 20 >= n < 40 Easy/Medium 40 >= n < 60 Medium 60 >= n < 80 Hard/Medium 80 >= n <= 100 Hard
	 *
	 * The difficulty classification as regard as period of the day. If period of the day is... GOOD: 00 >= n < 20 0 (the acumulative value) 20 >= n < 40 25 40
	 * >= n < 60 50 60 >= n < 80 75 80 >= n <= 100 100
	 *
	 * MEDIUM: 00 >= n < 20 25 20 >= n < 40 75 40 >= n < 60 100 60 >= n < 80 75 80 >= n <= 100 25
	 *
	 * EASY: 00 >= n < 20 100 20 >= n < 40 75 40 >= n < 60 50 60 >= n < 80 25 80 >= n <= 100 0
	 *
	 * @param periodAvailable
	 *            if is GOOD, MEDIUM or EASY.
	 * @param dificultyAverage
	 *            the difficulty average of the all subjects.
	 *
	...
	/**
	 * Search if the period contains subjects that have distinct difficulty. In other words, if exist subjects hard and easy in the same period.
	 *
	 * @param subjectWorkloads
	 *            the period.
	 *
	 * @return <code>true</code> if exist. <br/>
	...
	/**
	 * Generate individuals that contains the max number of distinct genes. <br/>
	 *
	 * <br/>
	 * Classification: Fixed Constraint<br/>
	 *
	 * @param individual
	 *            the current individual to be verified.
	 *
	...
	/**
	 * The cumulative value is then given by subtracting 100 by the percentage of similarity between the quantity of equal periods and total of the periods not
	 * empty. This means that if the plan has repeated 10 times and 100 non-peak periods, the percentage of closeness is 10%, then the cumulative value is 90,
	 * which is the subtraction 100 - 10.
	 *
	 * @param qttRepetitivePeriods
	 *            quantity of equal periods.
	 * @param totalPeriods
	 *            total of the periods not empty.
	 *
	 ...
	 *            period of the day B
	 *
	 * @return <code>true</code> if the periods are equals. <br/>
	 *         <code>false</code> otherwise.
	 *
	 */
	public boolean equalPeriods(SubjectWorkload[] periodA, SubjectWorkload[] periodB) {
		...
		for (int i = 0; i < periodA.length; i++) {
			if (periodA[i].getSubject().getId() != periodB[i].getSubject().getId())
				return false;
			if (periodA[i].getWorkload() != periodB[i].getWorkload())
				return false;
		}
	...
	/**
	 * Try to fill the period of the day that the user have disponibility to study. <br/>
	 *
	 * <br/>
	 * Classification: Fixed <br/>
	 *
	...
	/**
	 * Get the correct acumulativeValue from a list of subjectWorkloads and the period of the day available. <br/>
	 *
	 * <br/>
	 * Classification table: <br/>
	 *
	 * <br/>
	 * If period of the day is... <br/>
	 * NOTHING: <br/>
	...
	public int getAcumulativeValueByWorkload(int workloadSum, char dayPeriodAvailable) {
		...
		if (dayPeriodAvailable == NOTHING) {
			...
			if (workloadSum <= 1 || workloadSum >= 8) {
				acumulativeValue = 0;
			} else if (workloadSum >= 6 || workloadSum <= 2) {
				acumulativeValue = 40;
			} else {
				acumulativeValue = 100;
			}
	...
	/**
	 * The ideal is fill all time period avaible with a more subjects quantity.
	 *
	 * @param subjectWorkloads
	 *            [description]
	 * @param dayPeriodAvailable
	...
	/**
	 * Check if have subjects in the period of the day who the user don't have availability.<br/>
	 *
	 * <br/>
	 * Classification: Fixed Constraint.<br/>
	 *
	 * <br/>
	 * <b>NOTE</b>: Maybe should be better verify in relation of quantity of hours and not in relation of quantity of subjects.
	 *
	 * @param individual
	 *            the study plan.
	 *
	 * @return <code>true</code> if the constraint was attended. <br/>
	 *         <code>false</code> otherwise.
	 *
	 * @see Period
	 * @see DayPlanGene
	 ...
	 *
	 * <br/>
	 * WHEN A PERSON CHOISE "NOTHING" IN A PERIOD OF THE DAY, THEY WAS PLANING <br/>
	 *
	 * <br/>
	 * Classification: Soft Constraint. <br/>
	 *
	 * @param individual
	 *            the current individual to be verified.
	 *
	...
	/**
	 * Get a acumulativeValue from a individual respecting the hours to leisure from user.
	 *
	 * @param qtdTotalHoursAvailable
	 *            the total of hours available.
	 * @param hoursSum
	 *            sum of the all subjects in the individual.
	 * @param hoursToLeisure
	 *            hours reserved from user to not study.
	 *
	...
	/**
	 * Return the acumulative value in relation of the quantity of subjects by period. If the period have more than one alocated subject 100 is returned, 0
	 * otherwise. <br/>
	 *
	 * @param qttSubjectsByPeriod
	 *            the quantity of subjects by period.
	 *
	 * @return 100 if have more than 1; <br/>
	 *         0 otherwise.
	 */
	...
	/**
	 * Return the acumulative value in relation of the classification below: <br/>
	 *
	 * <br/>
	 * if the difference between max and the second max is: <br/>
	 * 0 then: 100 - (0*25) is equals 100. <br/>
	 * 1 then: 100 - (1*25) is equals 75. <br/>
	 * and so on.. <br/>
	 *
	 * @param period
	 *            the current period to be verified.
	 *
	...
	/**
	 * Check if exist periods that the sum of workload is greather than 6 hours.
	 *
	 * Classification: Fixed Constraint.
	 * 
	 * @param individual
	 * 
	 * @deprecated It's behavior is implemented in the individual creation.
	 *
	 * @return <code>true</code> if the constraint was attended. <code>false</code> otherwise.
	 */
	...
	/**
	 * Verify if all subjectsWorkload into the period of the day passed by param exceeds six hours.
	 *
	 * @param periodWithSubjects
	 *            the subjects of the period of the day.
	 *
	 * @return 100 if the subjects workload don't exceeds six hours. 0 if exceeds.
	 */
	...
	public void printInputConverted() {
		...
		for (Period period : studyCycle) {
			...
			System.out.println("" + cycleIt + 1 + " " + period.getMorning() + " " + period.getAfternoon() + " " + period.getNight());
